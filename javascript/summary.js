...
for(let key in user) {
    console.log(user.key, user[key], key) // undifined, kim, name ==> user.key 는 user.'key'으로 해석되기 때문. user.name이 되려면 user[key]로 사용해야함
}
//for of, for in 차이
// for(let i in user) ==> i는 프로퍼티명 즉 키, user[키]
// for(let i of user) ==> i는 값 즉 인덱스가 아닌 값, user[0]
